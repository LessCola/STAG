...
class GCN(nn.Module):
    ...
    def inference(self, g, x, batch_size, device):
        """
        Inference with the GCN model on full neighbors (i.e. without neighbor sampling).
        g : the entire graph.
        ...
        for l, layer in enumerate(self.gcn_layers):
            if l < self.num_layers - 1:
                ...
                # Final layer
                if not self.is_pretraining:  # Similar to GAT's emb=False case
                    y = torch.zeros(
                        g.num_nodes(),
                        self.num_classes,
                        device=device,
                    )
                else:  # Similar to GAT's emb=True case
                    y = torch.zeros(
                        g.num_nodes(),
                        self.out_dim,
                        device=device,
...
class GraphConv(nn.Module):
    ...
    def forward(self, graph, feat):
        with graph.local_scope():
            ...
            aggregate_fn = fn.copy_u("h", "m")  # ! correct
