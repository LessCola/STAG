...
from model.fewshot import zeroshot_without_llm
from model.prompt_quantizer import PromptVQ
...
def main(args, ways=None, shots=None):
    ...
    for dataset_idx, test_dataset_name in enumerate(test_dataset_names):
        ...
        prompt_quantizer = PromptVQ(args, test_labels, test_categories)
        model.prompt_quantizer = prompt_quantizer
        torch.cuda.empty_cache()

        for way in ways:
            print(f"####### Run {way}-way 0-shot")
            test_accs_list, val_accs_list = zeroshot_without_llm(
                model,
                test_graph,
                test_feats,
                test_nodes,
