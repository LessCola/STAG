...
class Tee:
    def __init__(self, log_file, mode="w"):
        self.terminal = sys.__stdout__  # Store the original terminal stdout
        self.log = open(log_file, mode)  # Open the log file
        self.terminal_stdout = self.terminal  # Expose the original terminal explicitly

    def write(self, message):
        self.terminal.write(message)  # Write to the terminal
        self.terminal.flush()  # Explicitly flush the terminal
        self.log.write(message)  # Write to the log file
        self.log.flush()  # Explicitly flush the log file
    ...
    def isatty(self):
        # Check if the original terminal is a TTY
        return self.terminal.isatty()

    def __getattr__(self, attr):
        # Delegate attribute access to the original terminal
...
def build_args():
    ...
    # * loss_fn: list of strings
    parser.add_argument("--loss_fn", type=str, nargs="+", default=["cont"])
    ...
    parser.add_argument("--commit_score", type=float, default=0.25)
    # * decoder
    parser.add_argument("--out_dim", type=int, default=768)
    # * few shot
    parser.add_argument("--n_way", type=int, default=0)
    parser.add_argument("--k_shot", type=int, default=5)
    # * feature fusion
    parser.add_argument("--fusion", type=str, default="weighted")
    # * llm model
    parser.add_argument(
        "--llm_model", type=str, default="meta-llama/Meta-Llama-3-8B-Instruct"
    )
...
# ! load config
def load_args_config(args, cfg_path):
    ...
    for k, v in config.items():
        if "lr" in k or "weight_decay" in k:
            v = float(v)
        setattr(args, k, v)
    logging.info(f"----- Using configs from {cfg_path} -----")
    return args
