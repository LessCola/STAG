...
from model.fewshot import zeroshot_with_llm
from model.quantizer import VQ
from model.zeroshot_classifier import ZeroShotClassifier
from src.config import load_config
from src.lc_sampler import load_single_dataset
from src.utils import Tee, build_args, load_args_config, set_random_seed
...
def main(args, ways=None, shots=None):
    ...
    # * LLM or embedding classifier
    fs_classifier = ZeroShotClassifier(args.llm_model, device)
    ...
    for dataset_idx, test_dataset_name in enumerate(test_dataset_names):
        ...
        val_accs[test_dataset_name] = {way: [] for way in ways}
        test_accs[test_dataset_name] = {way: [] for way in ways}
        ...
        for way in ways:
            print(f"####### Run {way}-way 0-shot")
            test_accs_list, val_accs_list = zeroshot_with_llm(
                model,
                fs_classifier,
                test_graph,
                test_feats,
            ...
            val_accs[test_dataset_name][way] = val_accs_list
            test_accs[test_dataset_name][way] = test_accs_list
            ...
            metric_name = f"{test_dataset_name}/{way}-way_0-shot"
            run.log(
                {
    ...
    for way in ways:
        val_acc = np.mean(
            [acc for dataset_dict in val_accs.values() for acc in dataset_dict[way]]
        )
        val_std = np.std(
            [acc for dataset_dict in val_accs.values() for acc in dataset_dict[way]]
        )
        print(f"- {way}-Way 0-Shot Val Acc: {val_acc:.4f}±{val_std:.4f}")
        test_acc = np.mean(
            [acc for dataset_dict in test_accs.values() for acc in dataset_dict[way]]
        )
        test_std = np.std(
            [acc for dataset_dict in test_accs.values() for acc in dataset_dict[way]]
        )
        print(f"- {way}-Way 0-Shot Test Acc: {test_acc:.4f}±{test_std:.4f}")
...
if __name__ == "__main__":
    ...
    with open(csv_file, "w") as f:
        ...
        for test_data in test_datasets:
            for way in ways:
                val_mean = np.mean(val_accs[test_data][way])
                val_std = np.std(val_accs[test_data][way])
                test_mean = np.mean(test_accs[test_data][way])
                test_std = np.std(test_accs[test_data][way])
                f.write(
                    f"{test_data},{way},0,{val_mean:.4f}±{val_std:.4f},{test_mean:.4f}±{test_std:.4f}\n"
                )
